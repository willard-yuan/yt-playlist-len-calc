...
export function ExporterContent() {
  ...
  const fetchPlaylistData = async (urlOverride?: string) => {
    ...
    if (!urlToFetch.trim()) {
      setError(subT("exporter.error.empty"))
      return
    }

    if (!validatePlaylistUrl(urlToFetch)) {
      setError(subT("exporter.error.invalid"))
      return
    }
        ...
        <div className="max-w-5xl mx-auto">
          ...
          <div className="relative z-20 bg-background/50 backdrop-blur-2xl rounded-3xl p-8 md:p-12 mb-24 shadow-[0_0_50px_-12px_rgba(0,0,0,0.12)] dark:shadow-[0_0_50px_-12px_rgba(255,255,255,0.05)] ring-1 ring-border/50">
            <div className="text-center mb-10">
              <h2 className="text-3xl font-bold mb-4 tracking-tight flex items-center justify-center gap-3">
                ...
                {subT("exporter.input.title")}
              </h2>
              <p className="text-muted-foreground text-lg">
                {subT("exporter.input.sub")}
              </p>
            </div>
            ...
            <div className="space-y-6">
              <div className="relative max-w-3xl mx-auto group">
                ...
                <div className="relative z-10">
                   ...
                   <Input
                     placeholder={subT("exporter.input.placeholder")}
                     value={playlistUrl}
                ...
                <div className="text-center mb-12">
                  ...
                  <div className="flex flex-wrap justify-center gap-4">
                    <Badge variant="secondary" className="px-4 py-2 text-sm bg-purple-100 dark:bg-purple-900/30 text-purple-700 dark:text-purple-300 border-0">
                      {fmt("exporter.result.videos", { count: playlistData.videoCount })}
                    </Badge>
                    <Badge variant="secondary" className="px-4 py-2 text-sm bg-blue-100 dark:bg-blue-900/30 text-blue-700 dark:text-blue-300 border-0">
                      {fmt("exporter.result.total", { total: playlistData.totalDuration })}
                    </Badge>
                  </div>
                </div>
                  ...
                  <div className="bg-background rounded-2xl p-6 border border-border/50 shadow-sm">
                    <div className="flex flex-col sm:flex-row items-center justify-between gap-6">
                      <div className="w-full sm:w-auto">
                        <h3 className="text-lg font-semibold mb-1">{subT("exporter.export.options")}</h3>
                        <p className="text-sm text-muted-foreground">{subT("exporter.export.optionsSub")}</p>
                      </div>
                        ...
                        <SelectContent>
                          <SelectItem value="csv">
                            <div className="flex items-center space-x-2">
                              ...
                              <span>{subT("exporter.format.csv")}</span>
                            </div>
                          </SelectItem>
                          <SelectItem value="excel">
                            <div className="flex items-center space-x-2">
                              <FileSpreadsheet className="h-4 w-4" />
                              <span>{subT("exporter.format.excel")}</span>
                            </div>
                          </SelectItem>
                          <SelectItem value="text">
                            <div className="flex items-center space-x-2">
                              <FileText className="h-4 w-4" />
                              <span>{subT("exporter.format.text")}</span>
                            </div>
                          </SelectItem>
                          <SelectItem value="bookmark">
                            <div className="flex items-center space-x-2">
                              <Bookmark className="h-4 w-4" />
                              <span>{subT("exporter.format.bookmark")}</span>
                            </div>
                          </SelectItem>
                        </SelectContent>
                      </Select>
                ...
                <div>
                  ...
                  {playlistData.videos.length > visibleCount && (
                    <div className="text-center mt-8">
                      ...
                      >
                        {fmt("exporter.more", { count: playlistData.videos.length - visibleCount })}
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-8">
            <div className="group p-8 rounded-3xl bg-secondary/30 hover:bg-secondary/50 transition-colors duration-300">
              ...
              <h3 className="text-xl font-semibold mb-3">{subT("exporter.feat.excel.title")}</h3>
              <p className="text-muted-foreground leading-relaxed text-sm">
                {subT("exporter.feat.excel.desc")}
              </p>
            </div>
            ...
            <div className="group p-8 rounded-3xl bg-secondary/30 hover:bg-secondary/50 transition-colors duration-300">
              ...
              <h3 className="text-xl font-semibold mb-3">{subT("exporter.feat.csv.title")}</h3>
              <p className="text-muted-foreground leading-relaxed text-sm">
                {subT("exporter.feat.csv.desc")}
              </p>
            </div>
            ...
            <div className="group p-8 rounded-3xl bg-secondary/30 hover:bg-secondary/50 transition-colors duration-300">
              ...
              <h3 className="text-xl font-semibold mb-3">{subT("exporter.feat.text.title")}</h3>
              <p className="text-muted-foreground leading-relaxed text-sm">
                {subT("exporter.feat.text.desc")}
              </p>
            </div>
            ...
            <div className="group p-8 rounded-3xl bg-secondary/30 hover:bg-secondary/50 transition-colors duration-300">
              ...
              <h3 className="text-xl font-semibold mb-3">{subT("exporter.feat.bookmark.title")}</h3>
              <p className="text-muted-foreground leading-relaxed text-sm">
                {subT("exporter.feat.bookmark.desc")}
              </p>
            </div>
          </div>
