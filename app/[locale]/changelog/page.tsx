...
export async function generateMetadata({
  ...
  const title = subT("seo.changelog.title");
  const description = subT("seo.changelog.desc");
  const canonicalUrl = `${SITE_URL}/${l}/changelog`;

  const languages: Record<string, string> = { en: `${SITE_URL}/changelog` };
  SUPPORTED_LOCALES.forEach((x) => {
    if (x !== "en") languages[x] = `${SITE_URL}/${x}/changelog`;
  });
  languages["x-default"] = `${SITE_URL}/changelog`;
...
export default function LocaleChangelog({
  params,
}: {
  params: { locale: string };
}) {
