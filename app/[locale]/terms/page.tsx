...
export async function generateMetadata({
  ...
  const title = subT("seo.terms.title");
  const description = subT("seo.terms.desc");
  const canonicalUrl = `${SITE_URL}/${l}/terms`;

  const languages: Record<string, string> = { en: `${SITE_URL}/terms` };
  SUPPORTED_LOCALES.forEach((x) => {
    if (x !== "en") languages[x] = `${SITE_URL}/${x}/terms`;
  });
  languages["x-default"] = `${SITE_URL}/terms`;
